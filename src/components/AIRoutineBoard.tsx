...
import { FaArrowUp, FaMinus, FaPlus, FaArrowLeft, FaArrowRight } from "react-icons/fa";
...
const CATEGORIES = [
  "Health", "Work", "Productivity", "Errands", "Chores",
  "Groceries", "Relax", "Sleep", "Meals", "Other",
] as const;
...
const validateAIPortal = (
  fromH: string, fromM: string, fromP: "AM" | "PM",
  toH: string, toM: string, toP: "AM" | "PM",
  name: string,
  tasks: AIRoutineItem[],
  excludeName?: string,
): string | null => {
  if (!name.trim()) return "Task name is required";
  const fH = parseInt(fromH); const tH = parseInt(toH);
  if (isNaN(fH) || fH < 1 || fH > 12) return "Start hour must be 1–12";
  const fM = parseInt(fromM); if (isNaN(fM) || fM < 0 || fM > 59) return "Start minute 00–59";
  if (isNaN(tH) || tH < 1 || tH > 12) return "End hour must be 1–12";
  const tM = parseInt(toM); if (isNaN(tM) || tM < 0 || tM > 59) return "End minute 00–59";
  const fromMins = portalTimeToMinutes(fromH, fromM, fromP);
  const toMins = toH === "12" && toM === "00" && toP === "AM" ? 1440 : portalTimeToMinutes(toH, toM, toP);
  if (toMins <= fromMins) return "End time must be after start time";
  if (toMins - fromMins < 5) return "Task must be at least 5 minutes";
  const hasDup = tasks.some(t => t.name !== excludeName && t.name.toLowerCase() === name.trim().toLowerCase());
...
const buildTasksWithGaps = (tasks: AIRoutineItem[]) => {
  ...
  for (const task of sorted) {
    ...
    if (start > prev) {
      result.push({ name: "dummy", isDummy: true, time: `${minutesToTime(prev)} - ${minutesToTime(start)}` });
    }
    result.push(task);
    prev = Math.min(1440, start + dur);
  }
  if (prev < 1440) {
    result.push({ name: "dummy", isDummy: true, time: `${minutesToTime(prev)} - 12:00 AM` });
  }
  return result;
};
...
export default function AIRoutineBoard() {
  ...
  const [weekRealDates, setWeekRealDates] = useState<Array<{
    dayName: string; shortDay: string; dateStr: string; isToday: boolean; dateObj: Date;
  }>>([]);

  // ── Chat state ─────────────────────────────────────────────
  const [chatHistory, setChatHistory] = useState<ChatSession[]>([]);
  const [activeDate, setActiveDate] = useState<string>(new Date().toLocaleDateString("en-CA"));
  const [inputText, setInputText] = useState("");
  const [isThinking, setIsThinking] = useState(false);
  const [showHistory, setShowHistory] = useState(false);
  const [hoveredTask, setHoveredTask] = useState<{ name: string; time: string; dur: number } | null>(null);
  const [cursorPos, setCursorPos] = useState<{ x: number; y: number }>({ x: 0, y: 0 });

  // ── Task alert state ───────────────────────────────────────
  const [taskAlert, setTaskAlert] = useState<{ name: string; time: string } | null>(null);
  ...
  const [portalMessage, setPortalMessage] = useState<{ type: "success" | "error"; text: string } | null>(null);
  const [dragState, setDragState] = useState<{
    taskName: string; dayKey: Day; taskStartMinutes: number; taskDuration: number;
    previewStartMinutes: number | null; previewDayKey?: Day;
  } | null>(null);
  ...
  const scrollAllChatsToBottom = () => {
    [chatScrollDesktopRef, chatScrollTabletRef, chatScrollMobileRef].forEach((ref) => {
      if (ref.current) {
  ...
  const updateAIRoutine = (newRoutine: AIRoutineData) => {
    ...
    if (auth?.email) upsertAIRoutine(auth.email, newRoutine).catch(console.error);
  };
  ...
  const activeMessages: ChatMessage[] = chatHistory.find((s) => s.date === activeDate)?.messages ?? [];

  // ── Premium check ──────────────────────────────────────────
  const MAX_RESPONSES = 30;
  const todayStr = new Date().toLocaleDateString("en-CA");

  const isPremium = (): boolean => (auth?.paymentType?.toLowerCase() ?? "").includes("premium");

  const getCurrentCount = (): number => {
    if (!auth?.todayPremiumResponses) return 0;
    const parts = auth.todayPremiumResponses.split(" ");
    if (parts.length !== 2 || parts[0] !== todayStr) return 0;
  ...
  useEffect(() => {
    ...
    const parts = auth.todayPremiumResponses?.split(" ") ?? [];
    if (parts.length === 2 && parts[0] === todayStr) return;
    const newValue = `${todayStr} 0`;
    setAuth({ ...auth, todayPremiumResponses: newValue });
    incrementTodayPremiumCount(auth.email, newValue).catch(console.error);
  ...
  useEffect(() => {
    const todayLower = new Date().toLocaleString("en-US", { weekday: "long" }).toLowerCase() as Day;
  ...
  const scrollToNow = () => {
    ...
    const c = refs.find((r) => r && r.scrollHeight > r.clientHeight) ?? refs.find((r) => r != null) ?? null;
    if (!c) return;
    const top = nowHeight - c.clientHeight / 2;
    const maxScroll = Math.max(0, c.scrollHeight - c.clientHeight);
    c.scrollTo({ top: Math.min(maxScroll, Math.max(0, top)), behavior: "smooth" });
  };

  // ── Rotate week ────────────────────────────────────────────
  const rotateWeekLeft = () => {
    setDaysOfWeekOrder((prev) => { const n = [...prev]; const last = n.pop()!; n.unshift(last); return n; });
  };
  const rotateWeekRight = () => {
    setDaysOfWeekOrder((prev) => { const n = [...prev]; const first = n.shift()!; n.push(first); return n; });
  };
  ...
  const openAddPortal = (day: Day, startStr?: string, endStr?: string) => {
    ...
    if (startStr) {
      ...
      setPortalFromHour(p.hour); setPortalFromMinute(p.minute); setPortalFromPeriod(p.period);
    } else { setPortalFromHour("9"); setPortalFromMinute("00"); setPortalFromPeriod("AM"); }
    if (endStr) {
      const p = parseTimeToPortal(endStr);
      setPortalToHour(p.hour); setPortalToMinute(p.minute); setPortalToPeriod(p.period);
    } else { setPortalToHour("10"); setPortalToMinute("00"); setPortalToPeriod("AM"); }
  };
  ...
  const openEditPortal = (day: Day, task: AIRoutineItem) => {
    ...
    setPortalFromHour(fp.hour); setPortalFromMinute(fp.minute); setPortalFromPeriod(fp.period);
    setPortalToHour(tp.hour); setPortalToMinute(tp.minute); setPortalToPeriod(tp.period);
    setPortalMessage(null);
  };
  ...
  const handlePortalAdd = () => {
    ...
    const err = validateAIPortal(
      portalFromHour, portalFromMinute, portalFromPeriod,
      portalToHour, portalToMinute, portalToPeriod,
      portalTaskName, dayTasks
    );
    if (err) { setPortalMessage({ type: "error", text: err }); return; }
    ...
    const newTask: AIRoutineItem = { name: portalTaskName.trim(), time: `${fromStr} - ${toStr}`, category: portalCategory };
  ...
  const handlePortalEdit = () => {
    ...
    const err = validateAIPortal(
      portalFromHour, portalFromMinute, portalFromPeriod,
      portalToHour, portalToMinute, portalToPeriod,
      portalTaskName, dayTasks, portalOriginalName
    );
    if (err) { setPortalMessage({ type: "error", text: err }); return; }
    const fromStr = `${portalFromHour.padStart(2,"0")}:${portalFromMinute.padStart(2,"0")} ${portalFromPeriod}`;
    const toStr = `${portalToHour.padStart(2,"0")}:${portalToMinute.padStart(2,"0")} ${portalToPeriod}`;
    const updated = {
      ...
      [portalDay]: dayTasks.map(t =>
        t.name === portalOriginalName
          ? { ...t, name: portalTaskName.trim(), time: `${fromStr} - ${toStr}`, category: portalCategory }
          : t
      ),
    };
    updateAIRoutine(updated);
  ...
  const handlePortalDelete = () => {
    ...
    if (!sure) return;
    const updated = { ...aiRoutine, [portalDay]: (aiRoutine[portalDay] || []).filter(t => t.name !== portalOriginalName) };
    updateAIRoutine(updated);
    closePortal();
  };
  ...
  const validateAIDrop = (
    routine: AIRoutineData, taskName: string, dayKey: Day,
    newStart: number, duration: number
  ...
  const sendMessage = async () => {
    ...
    setChatHistory((prev) => {
      ...
      if (existing) {
        return prev.map((s) => s.date === activeDate ? { ...s, messages: [...s.messages, userMsgObj, loadingMsg] } : s);
      }
      return [...prev, { date: activeDate, messages: [userMsgObj, loadingMsg] }];
    });

    const convForAI: [string, string][] = [];
    const msgs = activeMessages;
    for (let i = 0; i + 1 < msgs.length; i += 2) {
      if (msgs[i]?.role === "user" && msgs[i + 1]?.role === "ai" && msgs[i + 1].text !== "loading") {
        convForAI.push([msgs[i].text, msgs[i + 1].text]);
      }
    }

    try {
      const result = await aiRoutineResponse(userMsg, convForAI, JSON.stringify(auth.routine ?? {}), JSON.stringify(aiRoutine));
      const aiMsg: ChatMessage = { role: "ai", text: result.text, timestamp: new Date().toISOString() };
      ...
      const newValue = `${todayStr} ${newCount}`;
      setAuth({ ...auth, todayPremiumResponses: newValue });
      await incrementTodayPremiumCount(auth.email, newValue);
      if (result.updatedRoutine) {
        updateAIRoutine(result.updatedRoutine);
      }
    } catch {
      const errMsg: ChatMessage = { role: "ai", text: "Sorry, something went wrong. Please try again.", timestamp: new Date().toISOString() };
      setChatHistory((prev) => prev.map((s) => {
        if (s.date !== activeDate) return s;
        const updated = [...s.messages];
  ...
  const sortedHistoryDates = [...chatHistory].sort((a, b) => b.date.localeCompare(a.date)).map((s) => s.date);

  // ── Timeline column ────────────────────────────────────────
  const renderTimelineColumn = () => (
    <div className={`text-base pt-5 font-medium col-span-1 ${theme ? "text-gray-800" : "text-gray-200"}`}>
      {timeSlots.map((time) => (
        <div key={time} className={`w-full text-xs border-t ${theme ? "border-gray-400" : "border-gray-500"}`} style={{ height: `${hourHeight / slotsPerHour}px` }}>
          <div className="flex relative justify-center items-center">
            <div className={`absolute px-2 left-0 rounded-md text-xs font-medium shadow-sm ${theme ? "bg-white text-gray-700 border border-gray-200" : "bg-gray-900 text-gray-300 border border-gray-700"}`}>
              {time}
            </div>
            {(() => {
              const count = zoomLevel <= 1.5 ? 6 : zoomLevel <= 3 ? 3 : zoomLevel === 3.5 ? 10 : zoomLevel >= 4 ? 5 : 0;
              return Array.from({ length: count }).map((_, index) => (
                <div key={index} className={`w-[10%] absolute right-0 border-t ${theme ? "border-gray-400/70" : "border-gray-500/70"}`}
                  style={{ top: `${(hourHeight / slotsPerHour / count) * index - 1}px` }} />
              ));
            })()}
          </div>
        </div>
      ))}
      <div className={`w-full text-xs border-t ${theme ? "border-gray-400" : "border-gray-500"}`} style={{ height: "1px" }}>
        <div className="flex relative justify-center items-center">
          <div className={`absolute px-2 left-0 rounded-md text-xs font-medium shadow-sm ${theme ? "bg-white text-gray-700 border border-gray-200" : "bg-gray-900 text-gray-300 border border-gray-700"}`}>
            12:00 AM
          </div>
  ...
  // ── Task block ─────────────────────────────────────────────
  const renderTaskBlock = (task: AIRoutineItem & { isDummy?: boolean }, i: number, isToday: boolean, dayName?: string) => {
    const dur = getDuration(task.time);
    ...
    const isDragging = dragState?.taskName === task.name && dragState?.dayKey === dayKey;
    ...
    if (task.isDummy) {
      ...
      return (
        ...
        >
          {height >= 20 && (
            <div className={`absolute inset-[3px] rounded border-2 border-dashed flex items-center justify-center transition-opacity opacity-0 group-hover:opacity-100 ${
              theme ? "border-purple-300" : "border-purple-700"
            }`}>
              {height >= 36 && (
                <span className={`text-[9px] font-semibold flex items-center gap-1 ${theme ? "text-purple-400" : "text-purple-500"}`}>
                  <span>+</span>
          ...
          {height >= 12 && height < 20 && (
            <div className={`text-[8px] opacity-50 ml-2 mt-0.5`}>{formatDuration(dur)} Free</div>
          )}
        </div>
      );
    }
    ...
    return (
      <div
        ...
        className={`text-sm overflow-hidden border-t pr-2 transition-colors relative cursor-grab active:cursor-grabbing ${
          ...
          isToday
            ? theme ? "bg-purple-100/70 text-black border-l-2 border-purple-500/60" : "bg-purple-950/50 text-white border-l-2 border-purple-400/60"
            : theme ? "bg-gray-100/60 text-gray-800 hover:bg-gray-200/60" : "bg-gray-800/50 text-gray-200 hover:bg-gray-700/70 border-t-gray-800"
        }`}
        style={{ height: `${height}px` }}
        ...
        onMouseMove={isSmall ? (e) => setCursorPos({ x: e.clientX, y: e.clientY }) : undefined}
        ...
        {height < 12 ? null : height < 30 ? (
          <div className="font-medium text-[9px] truncate ml-2.5 flex items-center gap-1.5">
            {task.category && CATEGORY_DOT[task.category] && (
              <span className={`w-[7px] h-[7px] rounded-full flex-shrink-0 ${CATEGORY_DOT[task.category]}`} />
            )}
          ...
          <>
            <div className="flex items-center gap-1.5 ml-2.5 mt-1">
              {task.category && CATEGORY_DOT[task.category] && (
                <span className={`w-[8px] h-[8px] rounded-full flex-shrink-0 ${CATEGORY_DOT[task.category]}`} />
              )}
              <div className="text-[11px] font-medium break-words min-w-0">{task.name}</div>
            </div>
            <div className="text-[10px] opacity-70 ml-2.5 truncate">{task.time}</div>
          </>
        ) : (
          <>
            <div className="flex items-center gap-1.5 ml-2.5 mt-2">
              {task.category && CATEGORY_DOT[task.category] && (
                <span className={`w-[9px] h-[9px] rounded-full flex-shrink-0 ${CATEGORY_DOT[task.category]}`} />
              )}
              <div className="font-medium break-words min-w-0">{task.name}</div>
            </div>
            <div className="text-xs opacity-70 ml-2.5">{task.time}</div>
            <div className="text-[11px] opacity-60 ml-2.5">{formatDuration(dur)}</div>
          </>
        )}
      </div>
    );
  ...
  const renderMessage = (text: string, isLoading: boolean) => {
    ...
    if (isLoading) {
      return (
        <m.div className="flex items-center space-x-1.5" variants={typingVariants} animate="animate">
          {[0, 1, 2].map((i) => <m.span key={i} className="inline-block w-2 h-2 bg-current rounded-full" />)}
        </m.div>
      );
    }
    return text.split("[/n]").filter((p) => p.trim()).map((para, i) => {
      const lines = para.split("\n").filter((l) => l.trim());
      const hasBullets = lines.some((l) => l.trimStart().startsWith("* ") || l.trimStart().startsWith("- "));
      if (hasBullets) {
        return (
          <ul key={i} className="list-disc list-inside pl-3 my-2 space-y-1 text-xs sm:text-sm">
            {lines.map((line, li) => <li key={li}>{renderBold(line.replace(/^[\*\-]\s*/, "").trim())}</li>)}
          </ul>
        );
      }
      const isBold = /^\*\*.*\*\*$/.test(para.trim()) || /^#{1,3}/.test(para.trim());
      const clean = para.replace(/^#{1,6}\s*/, "").trim();
      return (
        <p key={i} className={`text-xs my-1.5 ${isBold ? "font-bold text-purple-500" : ""}`}>
          {renderBold(clean)}
        </p>
      );
    });
  ...
  const renderChatContent = (scrollRef?: React.RefObject<HTMLDivElement>) => {
    if (!isPremium()) {
      return (
        <div className="flex-1 flex flex-col items-center justify-center text-center px-4 py-8">
          ...
          <h4 className={`text-sm font-bold mb-2 ${theme ? "text-gray-800" : "text-gray-200"}`}>Premium Feature</h4>
          <p className={`text-xs leading-relaxed mb-4 ${theme ? "text-gray-500" : "text-gray-500"}`}>
            Copy your routine here and refine it with a specialized AI daily routine expert. Get personalized suggestions, chat interactively, and seamlessly replace your routine with your optimized AI routine.
          </p>
          <a href="/pricing" className="inline-flex items-center px-4 py-2 rounded-lg text-xs font-medium bg-purple-600 hover:bg-purple-700 text-white transition">
            View Premium Plans →
          </a>
        </div>
      );
    ...
    if (showHistory) {
      return (
        <div className="flex-1 overflow-y-auto">
          <div className={`px-3 py-2 text-[10px] font-semibold uppercase tracking-wide border-b ${theme ? "text-gray-400 border-gray-200" : "text-gray-600 border-gray-800"}`}>
            Past Conversations
          </div>
          {sortedHistoryDates.length === 0 ? (
            <div className={`px-3 py-8 text-center text-[10px] ${theme ? "text-gray-400" : "text-gray-600"}`}>No conversations yet</div>
            ...
            return (
              <div key={date} className={`flex items-stretch border-b ${isActive ? theme ? "bg-purple-50 border-purple-100" : "bg-purple-950/30 border-purple-900/30" : theme ? "border-gray-100" : "border-gray-800/50"}`}>
                <button
                  onClick={() => { setActiveDate(date); setShowHistory(false); }}
                  className={`flex-1 text-left px-3 py-2.5 transition ${isActive ? "" : theme ? "hover:bg-gray-50" : "hover:bg-gray-900"}`}
                >
                  <div className="flex items-center justify-between mb-0.5">
                    <span className={`text-[11px] font-semibold ${isActive ? theme ? "text-purple-700" : "text-purple-400" : theme ? "text-gray-700" : "text-gray-300"}`}>
                      {formatDateLabel(date)}
                    </span>
                    <span className={`text-[9px] ${theme ? "text-gray-400" : "text-gray-600"}`}>{msgCount} msg{msgCount !== 1 ? "s" : ""}</span>
                  </div>
                  {lastMsg && (
                    <p className={`text-[10px] truncate ${theme ? "text-gray-500" : "text-gray-600"}`}>
                      {lastMsg.role === "user" ? "You: " : "AI: "}
                      {lastMsg.text.replace(/\*\*/g, "").replace(/\[\/n\]/g, " ").slice(0, 50)}
                ...
                <button
                  onClick={async () => {
                    if (!auth?.email) return;
                    const confirmed = confirm(`Delete conversation from ${formatDateLabel(date)}?`);
                    if (!confirmed) return;
                    setChatHistory((prev) => prev.filter((s) => s.date !== date));
                    if (isActive) {
                      const remaining = sortedHistoryDates.filter((d) => d !== date);
                      setActiveDate(remaining.length > 0 ? remaining[0] : new Date().toLocaleDateString("en-CA"));
                    }
          ...
          <button onClick={() => { setActiveDate(new Date().toLocaleDateString("en-CA")); setShowHistory(false); }}
    ...
    return (
      <>
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-2 py-3 space-y-1">
          {activeMessages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-3">
              ...
              <h3 className={`text-xs font-bold mb-1.5 ${theme ? "text-gray-800" : "text-gray-200"}`}>AI Routine Builder</h3>
              <p className={`text-[10px] leading-relaxed mb-3 ${theme ? "text-gray-500" : "text-gray-500"}`}>Talk to AI to build and update your routine.</p>
              <div className="space-y-1.5 w-full">
                {["Build a productive morning", "Copy my real routine", "Optimize for better sleep"].map((s) => (
            ...
            <>
              <div className="flex items-center gap-2 mb-2">
                <div className={`flex-1 h-px ${theme ? "bg-gray-200" : "bg-gray-800"}`} />
                <span className={`text-[9px] px-2 font-medium ${theme ? "text-gray-400" : "text-gray-600"}`}>{formatDateLabel(activeDate)}</span>
                <div className={`flex-1 h-px ${theme ? "bg-gray-200" : "bg-gray-800"}`} />
              </div>
              {activeMessages.map((msg, i) => {
                const isLoadingMsg = msg.text === "loading" && i === activeMessages.length - 1 && isThinking;
                return (
                  <div key={i} className="mb-2">
                    {msg.role === "user" ? (
                      <div className="flex flex-col items-end gap-0.5">
                        <div className={`max-w-[85%] px-2.5 py-1.5 rounded-xl rounded-tr-sm text-xs leading-relaxed ${theme ? "bg-purple-600 text-white" : "bg-purple-700 text-white"}`}>
                          {msg.text}
                        </div>
                        <span className={`text-[9px] mr-1 ${theme ? "text-gray-400" : "text-gray-600"}`}>{formatTime(msg.timestamp)}</span>
                      </div>
                    ) : (
                      <div className="flex flex-col items-start gap-0.5">
                        <div className={`max-w-[92%] px-2.5 py-1.5 rounded-xl rounded-tl-sm text-xs leading-relaxed border ${theme ? "bg-gray-50 border-gray-200 text-gray-800" : "bg-gray-900 border-gray-800 text-gray-200"}`}>
                          {renderMessage(msg.text, isLoadingMsg)}
                        </div>
                        {!isLoadingMsg && <span className={`text-[9px] ml-1 ${theme ? "text-gray-400" : "text-gray-600"}`}>{formatTime(msg.timestamp)}</span>}
                      </div>
                    )}
                  </div>
                );
        ...
        {/* Input */}
        <div className={`flex-shrink-0 p-2 border-t ${theme ? "border-gray-200" : "border-gray-800"}`}>
          {isLimitReached ? (
            <div className={`text-center py-2 px-2 text-[10px] rounded border ${theme ? "bg-red-50 border-red-200 text-red-600" : "bg-red-950/30 border-red-800 text-red-400"}`}>
              Today&apos;s limit reached ({MAX_RESPONSES} responses).<br />Try again tomorrow.
            </div>
          ) : (
            <>
              <div className={`relative flex items-end gap-1.5 p-1.5 rounded-lg border ${theme ? "bg-white border-gray-300" : "bg-gray-950 border-gray-700"}`}>
                <textarea
                  value={inputText} onChange={(e) => setInputText(e.target.value)}
                  onKeyDown={handleKey} placeholder={isThinking ? "Thinking..." : "Ask AI to update your routine..."}
                  rows={2}
                ...
                />
                <button onClick={sendMessage} disabled={!inputText.trim() || isThinking}
              ...
              <div className={`text-right text-[9px] mt-1 ${theme ? "text-gray-400" : "text-gray-600"}`}>
                {getCurrentCount()} / {MAX_RESPONSES} today
              </div>
            </>
          )}
        </div>
    ...
    return (
      <div className="fixed inset-0 bg-black flex items-center justify-center z-50">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-white tracking-wide mb-3">AI Routine</h2>
  ...
  if (!auth) {
    return (
      <div className={`min-h-screen flex items-center justify-center ${theme ? "bg-white" : "bg-black"}`}>
        <p className={`text-lg ${theme ? "text-gray-700" : "text-gray-300"}`}>Please log in to use AI Routine.</p>
      </div>
    );
  }

  // ── Sidebar toggle button (same pattern as DashBoard) ──────
  const SidebarToggle = ({ top, left, size = 30 }: { top: string; left: string; size?: number }) => (
    <motion.div
      ...
      whileTap={{ scale: 0.92 }}
    >
      {isSidebarOpen ? <MdKeyboardDoubleArrowLeft size={20} /> : <MdKeyboardDoubleArrowRight size={20} />}
    </motion.div>
  );
  ...
  const renderChatSidebar = (scrollRef?: React.RefObject<HTMLDivElement>) => (
    <div className="h-full w-full flex flex-col">
      {/* Header */}
      <div className={`flex items-center justify-between px-3 h-[50px] flex-shrink-0 border-b ${theme ? "border-gray-200" : "border-gray-800"}`}>
        <div className="flex items-center gap-2 w-full justify-end">
          {!isPremium() && (
            <span className={`text-sm font-bold ${theme ? "text-purple-700" : "text-purple-400"}`}>✦ Your AI Routine</span>
          )}
        </div>
        {isPremium() && (
            ...
            <button
              onClick={async () => {
                ...
                const newRoutine: AIRoutineData = {
                  saturday: (auth.routine?.saturday ?? []).map((t) => ({ name: t.name, time: t.time, category: t.category ?? "" })),
                  sunday: (auth.routine?.sunday ?? []).map((t) => ({ name: t.name, time: t.time, category: t.category ?? "" })),
                  monday: (auth.routine?.monday ?? []).map((t) => ({ name: t.name, time: t.time, category: t.category ?? "" })),
                  tuesday: (auth.routine?.tuesday ?? []).map((t) => ({ name: t.name, time: t.time, category: t.category ?? "" })),
                  wednesday: (auth.routine?.wednesday ?? []).map((t) => ({ name: t.name, time: t.time, category: t.category ?? "" })),
                  thursday: (auth.routine?.thursday ?? []).map((t) => ({ name: t.name, time: t.time, category: t.category ?? "" })),
                  friday: (auth.routine?.friday ?? []).map((t) => ({ name: t.name, time: t.time, category: t.category ?? "" })),
                };
    ...
    <>
        ...
        >
          ...
          >
            ...
            <div className="sticky top-0 left-0 right-0 z-30 bg-inherit">
              <div className={`h-[50px] w-full border-b-[1px] flex items-center justify-between px-6 ${theme ? "border-gray-300" : "border-gray-800"}`}>
                {/* Zoom */}
                <div className="flex items-center gap-2 sm:gap-3">
                  <button onClick={() => setZoomLevel((p) => Math.max(1, p - 0.5))} disabled={zoomLevel <= 1}
                  ...
                  <span className={`font-semibold min-w-[90px] text-center text-sm sm:text-base ${theme ? "text-gray-900" : "text-white"}`}>
                    Zoom: {zoomLevel.toFixed(1)}x
                  </span>
                  <button onClick={() => setZoomLevel((p) => Math.min(8, p + 0.5))} disabled={zoomLevel >= 8}
                ...
                <div className="flex items-center gap-2 sm:gap-4">
                  <button onClick={rotateWeekRight}
                  ...
                  <span className={`font-medium text-sm sm:text-base ${theme ? "text-gray-900" : "text-white"}`}>Rotate Week</span>
                  <button onClick={rotateWeekLeft}
                    className={`p-1.5 rounded-md transition-colors flex items-center justify-center ${theme ? "text-purple-700 border-[1px] hover:text-purple-800 hover:bg-purple-50/70 bg-purple-50/70 border-purple-800/20" : "text-purple-400 border-[1px] hover:text-purple-300 hover:bg-purple-950/40 bg-purple-950/40 border-purple-300/20"}`}>
              ...
              {/* Day headers */}
              <div className={`h-[49px] w-full border-b ${theme ? "border-gray-300" : "border-gray-800"}`}>
                <div className="grid grid-cols-1 sm:grid-cols-8 mx-auto h-full">
                  <div className="overflow-hidden">
                    <div className={`h-full flex items-center justify-center font-semibold text-sm sm:text-base ${theme ? "text-gray-700 bg-gray-100/40" : "text-gray-400 bg-gray-900/40"}`}>
                      Timeline
                    </div>
                  </div>
                  {daysOfWeekOrder.map((day, idx) => (
                    <div key={day.full}
                      ...
                      <div className={`h-full flex items-center justify-center font-semibold text-sm sm:text-base transition-all duration-150 ${day.full === today ? theme ? "bg-purple-600/90 text-white shadow-sm" : "bg-purple-700/90 text-white shadow-sm" : theme ? "text-gray-800" : "text-gray-300"}`}>
                        {day.full}
                      </div>
                    </div>
                  ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-8 mx-auto relative">
                ...
                return (
                  <div key={day.full}
                    ...
                    onDragOver={(e) => {
                      ...
                      const clamped = Math.max(0, Math.min(1440 - dragState.taskDuration, snapped));
                      setDragState(prev => prev ? { ...prev, previewStartMinutes: clamped, previewDayKey: dayKey } : prev);
                    }}
                    onDrop={(e) => {
                      e.preventDefault();
                      if (!dragState) return;
                      ...
                      const dropY = e.clientY - rect.top - 20 * pxPerMinute;
                      const snapped = Math.round(dropY / pxPerMinute / 5) * 5;
                      const newStart = Math.max(0, Math.min(1440 - dragState.taskDuration, snapped));
                      const updated = { ...aiRoutine };
                      if (targetDayKey !== dragState.dayKey) {
                        const targetTasks = updated[targetDayKey] || [];
                        if (targetTasks.some(t => t.name === dragState.taskName)) {
                          alert(`"${dragState.taskName}" already exists on ${day.full}`);
                          setDragState(null); return;
                        }
                        const check = validateAIDrop(updated, dragState.taskName, targetDayKey, newStart, dragState.taskDuration);
                        if (!check.valid) { alert(`Overlaps with "${check.conflict}" on ${day.full}`); setDragState(null); return; }
                        const sourceTasks = updated[dragState.dayKey] || [];
                        const taskToMove = sourceTasks.find(t => t.name === dragState.taskName);
                        updated[dragState.dayKey] = sourceTasks.filter(t => t.name !== dragState.taskName);
                        if (taskToMove) {
                          updated[targetDayKey] = [...targetTasks, { ...taskToMove, time: `${minutesToTime(newStart)} - ${minutesToTime(newStart + dragState.taskDuration)}` }]
                            .sort((a, b) => timeToMinutes(a.time.split(" - ")[0]) - timeToMinutes(b.time.split(" - ")[0]));
                        }
                      } else {
                        const check = validateAIDrop(updated, dragState.taskName, dayKey, newStart, dragState.taskDuration);
                        if (!check.valid) { alert(`Overlaps with "${check.conflict}"`); setDragState(null); return; }
                        updated[dayKey] = (updated[dayKey] || []).map(t =>
                          t.name === dragState.taskName
                            ? { ...t, time: `${minutesToTime(newStart)} - ${minutesToTime(newStart + dragState.taskDuration)}` }
                            : t
                        ).sort((a, b) => timeToMinutes(a.time.split(" - ")[0]) - timeToMinutes(b.time.split(" - ")[0]));
                      }
                      updateAIRoutine(updated);
                  ...
                  >
                    <div className="relative">
                      {tasks.map((task, i) => renderTaskBlock(task, i, day.full === today, day.full))}
                      {dragState && (dragState.previewDayKey ?? dragState.dayKey) === dayKey && dragState.previewStartMinutes !== null && (
                        <div
                          className={`absolute left-0 right-0 pointer-events-none z-20 border-2 border-dashed rounded ${
                            theme ? "bg-purple-400/20 border-purple-500" : "bg-purple-500/20 border-purple-400"
                          }`}
                          style={{ top: `${dragState.previewStartMinutes * pxPerMinute}px`, height: `${dragState.taskDuration * pxPerMinute}px` }}
                        >
                          <div className={`text-[10px] font-semibold px-2 py-0.5 rounded-sm mt-1 ml-1 inline-block ${
                            theme ? "bg-purple-500 text-white" : "bg-purple-400 text-gray-900"
                          }`}>
                            {minutesToTime(dragState.previewStartMinutes)} → {minutesToTime(dragState.previewStartMinutes + dragState.taskDuration)}
                          </div>
                        </div>
                      )}
                    </div>
              ...
              {/* Now line */}
              <div className={`absolute w-[90%] ml-[10%] h-[2px] border-t-2 z-20 ${theme ? "border-purple-600" : "border-purple-500"}`} style={{ top: `${nowHeight - 1}px` }} />
              <div className={`absolute text-xs font-bold px-2.5 py-1 rounded-md left-[6.8%] z-20 shadow-sm border ${theme ? "bg-purple-600 text-white border-purple-700" : "bg-purple-700 text-white border-purple-600"}`} style={{ top: `${nowHeight - 13}px` }}>
                Now
              </div>
            </div>
          </div>
      ...
      <div className="hidden sm:block lg:hidden w-full overflow-hidden fixed" style={{ top: "56px", height: "calc(100% - 56px)" }}>
        ...
        <motion.div
          ...
          animate={{ width: isSidebarOpen ? sidebarTablet.open : sidebarTablet.closed }}
          transition={transition}
        >
          <AnimatePresence>
            {isSidebarOpen && (
        ...
        <motion.div
          className="h-full relative float-left"
          initial={{ width: "100%" }}
          animate={{ width: isSidebarOpen ? mainTablet.open : mainTablet.closed }}
          style={{ width: "100%" }}
          transition={transition}
        >
          <div
          ...
          >
            ...
            <div className="sticky top-0 left-0 z-30 bg-inherit">
              <div className={`h-[50px] w-full border-b-[1px] flex items-center justify-between ${theme ? "border-gray-300" : "border-gray-800"}`}>
                <div className="flex items-center gap-2 sm:gap-3">
                  <button onClick={() => setZoomLevel((p) => Math.max(1, p - 0.5))} disabled={zoomLevel <= 1}
                    className={`p-1.5 sm:p-2 rounded transition-colors flex items-center justify-center ${theme ? zoomLevel >= 8 ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "text-purple-700 hover:text-purple-800 hover:bg-purple-50/70 bg-purple-50/70 border border-purple-800/20" : zoomLevel >= 8 ? "bg-gray-700 text-gray-500 cursor-not-allowed" : "text-purple-400 hover:text-purple-300 hover:bg-purple-950/40 bg-purple-950/40 border border-purple-300/20"}`}>
                    <FaMinus size={10} />
              ...
              {/* Date capsules */}
              <div className={`w-full py-2 border-b flex items-center justify-between gap-1.5 sm:gap-2.5 ${theme ? "border-gray-300" : "border-gray-800"}`}>
                {weekRealDates.map((d, idx) => {
                  const isSelected = selectedDateIndex === idx;
                  return (
                    <button key={idx}
                      onClick={() => { setSelectedDay(d.dayName.toLowerCase() as Day); setSelectedDateIndex(idx); }}
                      ...
                      <div className="font-semibold tracking-tight">{d.shortDay}</div>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="grid grid-cols-2 mx-auto relative w-full">
                ...
                return (
                  <div
                    ...
                    onDragOver={(e) => {
                      ...
                      const snapped = Math.round(dropY / pxPerMinute / 5) * 5;
                      const clamped = Math.max(0, Math.min(1440 - dragState.taskDuration, snapped));
                      setDragState(prev => prev ? { ...prev, previewStartMinutes: clamped } : prev);
                    }}
                    onDrop={(e) => {
                      e.preventDefault();
                      if (!dragState || dragState.dayKey !== dayKey) return;
                      ...
                      const snapped = Math.round(dropY / pxPerMinute / 5) * 5;
                      const newStart = Math.max(0, Math.min(1440 - dragState.taskDuration, snapped));
                      const check = validateAIDrop(aiRoutine, dragState.taskName, dayKey, newStart, dragState.taskDuration);
                      if (!check.valid) { alert(`Overlaps with "${check.conflict}"`); setDragState(null); return; }
                      const updated = { ...aiRoutine, [dayKey]: (aiRoutine[dayKey] || []).map(t =>
                        t.name === dragState.taskName
                          ? { ...t, time: `${minutesToTime(newStart)} - ${minutesToTime(newStart + dragState.taskDuration)}` }
                          : t
                      ).sort((a, b) => timeToMinutes(a.time.split(" - ")[0]) - timeToMinutes(b.time.split(" - ")[0])) };
                  ...
                  >
                    <div className="relative">
                      {tasks.map((task, i) => renderTaskBlock(task, i, dayInfo.isToday, dayInfo.dayName))}
                      {dragState && dragState.dayKey === dayKey && dragState.previewStartMinutes !== null && (
                        <div
                          className={`absolute left-0 right-0 pointer-events-none z-20 border-2 border-dashed rounded ${
                            theme ? "bg-purple-400/20 border-purple-500" : "bg-purple-500/20 border-purple-400"
                          }`}
              ...
              <div className={`absolute left-[35%] right-0 h-[2px] border-t-2 z-20 ${theme ? "border-purple-600" : "border-purple-500"}`} style={{ top: `${nowHeight - 1}px` }} />
              <div className={`absolute text-xs font-bold px-2.5 py-1 rounded-md left-[32%] z-20 shadow-sm border ${theme ? "bg-purple-600 text-white border-purple-700" : "bg-purple-700 text-white border-purple-600"}`} style={{ top: `${nowHeight - 13}px` }}>
                Now
              </div>
            </div>
          </div>
      ...
      <div className="block sm:hidden w-full overflow-hidden fixed" style={{ top: "56px", height: "calc(100% - 56px)" }}>
        ...
        <motion.div
          ...
          animate={{ width: isSidebarOpen ? sidebarMobile.open : sidebarMobile.closed }}
          transition={transition}
        >
          <AnimatePresence>
            {isSidebarOpen && (
        ...
        <motion.div
          className="h-full relative float-left"
          initial={{ width: "100%" }}
          animate={{ width: isSidebarOpen ? mainMobile.open : mainMobile.closed }}
          style={{ width: "100%" }}
          transition={transition}
        >
          <div
          ...
          >
            ...
            {!isSidebarOpen && (
              <div className="sticky top-0 left-0 z-30 bg-inherit">
                <div className={`h-[55px] pb-[3px] w-full border-b-[1px] flex items-center justify-between ${theme ? "border-gray-300" : "border-gray-800"}`}>
                  <div className="flex items-center gap-1.5 sm:gap-3">
                    <button onClick={() => setZoomLevel((p) => Math.max(1, p - 0.5))} disabled={zoomLevel <= 1}
                      className={`p-1 rounded transition-colors flex items-center justify-center ${theme ? zoomLevel >= 8 ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "text-purple-700 hover:text-purple-800 hover:bg-purple-50/70 bg-purple-50/70 border border-purple-800/20" : zoomLevel >= 8 ? "bg-gray-700 text-gray-500 cursor-not-allowed" : "text-purple-400 hover:text-purple-300 hover:bg-purple-950/40 bg-purple-950/40 border border-purple-300/20"}`}>
                      <FaMinus size={10} />
                    </button>
                    <span className={`font-semibold min-w-[70px] text-center text-xs ${theme ? "text-gray-900" : "text-white"}`}>
                      Zoom: {zoomLevel.toFixed(1)}x
                    </span>
                    <button onClick={() => setZoomLevel((p) => Math.min(8, p + 0.5))} disabled={zoomLevel >= 8}
                      className={`p-1 rounded transition-colors flex items-center justify-center ${theme ? zoomLevel >= 8 ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "text-purple-700 hover:text-purple-800 hover:bg-purple-50/70 bg-purple-50/70 border border-purple-800/20" : zoomLevel >= 8 ? "bg-gray-700 text-gray-500 cursor-not-allowed" : "text-purple-400 hover:text-purple-300 hover:bg-purple-950/40 bg-purple-950/40 border border-purple-300/20"}`}>
                ...
                {/* Date capsules */}
                <div className={`w-full py-2 border-b flex items-center justify-between gap-1.5 ${theme ? "border-gray-300" : "border-gray-800"}`}>
                  {weekRealDates.map((d, idx) => {
                    const isSelected = selectedDateIndex === idx;
                    return (
                      <button key={idx}
            ...
            {/* Single day grid */}
            <div className={`grid ${isSidebarOpen ? "grid-cols-1" : "grid-cols-2"} mx-auto relative w-full`}>
              ...
              {!isSidebarOpen && (
                <>
                  <div className={`absolute left-[35%] right-0 h-[2px] border-t-2 z-20 ${theme ? "border-purple-600" : "border-purple-500"}`} style={{ top: `${nowHeight - 1}px` }} />
                  <div className={`absolute text-xs font-bold px-2.5 py-1 rounded-md left-[27%] z-20 shadow-sm border ${theme ? "bg-purple-600 text-white border-purple-700" : "bg-purple-700 text-white border-purple-600"}`} style={{ top: `${nowHeight - 13}px` }}>
                    Now
                  </div>
                </>
              )}
    ...
    {/* ── Hover tooltip for small tasks ── */}
      {hoveredTask && (
        ...
        >
          ...
          <div className={`text-[10px] ${theme ? "text-gray-400" : "text-gray-500"}`}>{formatDuration(hoveredTask.dur)}</div>
        </div>
      )}
      ...
      {taskAlert && (
        <div className="fixed z-50 bottom-5 left-5 w-[90vw] sm:w-[340px]">
          <div className={`relative rounded-2xl shadow-2xl border px-5 py-4 ${
            theme ? "bg-white border-purple-500 text-gray-900" : "bg-gray-900 border-purple-500 text-gray-100"
          }`}>
            <button
            ...
            <p className={`text-base font-bold mt-0.5 ${theme ? "text-purple-600" : "text-purple-400"}`}>
              {taskAlert.name}
            </p>
      ...
      {portalMode && (
        <div className={`fixed bottom-4 right-4 z-50 w-[300px] rounded-xl shadow-2xl border text-sm ${
          theme ? "bg-white border-gray-200 text-gray-800" : "bg-gray-950 border-gray-700 text-gray-100"
        }`}>
          {/* Header */}
          <div className={`flex items-center justify-between px-3 py-2.5 border-b rounded-t-xl ${
            ...
            <div className="flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${portalMode === "add" ? "bg-purple-500" : "bg-blue-500"}`} />
              <span className="text-xs font-semibold capitalize">
                {portalMode === "add" ? "Add Task" : "Edit Task"} · {portalDay}
              </span>
            </div>
            <button onClick={closePortal} className="text-gray-400 hover:text-red-500 font-bold text-base leading-none transition">×</button>
          </div>
          ...
          <div className="p-3 space-y-3">
            ...
            <input
              ...
              onKeyDown={e => { if (e.key === "Enter") { portalMode === "add" ? handlePortalAdd() : handlePortalEdit(); } }}
            ...
            <div className="flex flex-wrap gap-1">
              {CATEGORIES.map(cat => (
                <button
                  key={cat}
                  onClick={() => setPortalCategory(portalCategory === cat ? "" : cat)}
                  className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium transition border ${
                    portalCategory === cat
                      ? "bg-purple-600 border-purple-500 text-white"
                      : theme ? "bg-gray-100 border-gray-300 text-gray-600 hover:bg-gray-200" : "bg-gray-800 border-gray-700 text-gray-400 hover:bg-gray-700"
                  }`}
                >
                  {CATEGORY_DOT[cat] && <span className={`w-[6px] h-[6px] rounded-full ${CATEGORY_DOT[cat]}`} />}
                  {cat}
                </button>
              ))}
            </div>
            ...
            {(["From", "To"] as const).map(label => {
              ...
              const setMinute = isFrom ? setPortalFromMinute : setPortalToMinute;
              const setPeriod = isFrom ? setPortalFromPeriod : setPortalToPeriod;
              return (
                <div key={label}>
                  <div className="text-[10px] opacity-60 mb-1">{label}</div>
                  <div className="grid grid-cols-3 gap-1.5">
                    <input type="text" placeholder="HH" maxLength={2} value={hour}
                    ...
                    />
                    <input type="text" placeholder="MM" maxLength={2} value={minute}
            ...
            {/* Status message */}
            {portalMessage && (
              <div className={`text-[10px] text-center py-1 rounded ${
                portalMessage.type === "success"
                  ? theme ? "bg-green-100 text-green-700" : "bg-green-900/40 text-green-300"
                  : theme ? "bg-red-100 text-red-700" : "bg-red-900/40 text-red-300"
              }`}>
                {portalMessage.text}
              </div>
            )}
            ...
            <div className="flex gap-1.5">
              ...
              <button onClick={closePortal}
                className={`py-1.5 px-3 rounded text-xs font-semibold border transition ${
                  theme ? "border-gray-300 text-gray-600 hover:bg-gray-100" : "border-gray-700 text-gray-400 hover:bg-gray-800"
                }`}>
                Cancel
              </button>
            </div>
