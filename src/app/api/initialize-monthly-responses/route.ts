...
export async function POST() {
  try {
    ...
    console.log(`Found ${premiumUsers.length} premium users without monthly tracking`);
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      message: premiumUsersWithoutMonthly > 0 ? "Ready to initialize" : "All premium users initialized"
    });
