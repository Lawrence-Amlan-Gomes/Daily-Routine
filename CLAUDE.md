...
- **Fallback:** If `paddleSubscriptionId` not stored, `cancelSubscription` resolves the Paddle customer by email (`GET /customers?search=`), then fetches that customer's active subscription (`GET /subscriptions?customer_id=&status=active`) and cancels it (sending `effective_from: "next_billing_period"`), backfilling the id. The action **returns `{ error }` rather than throwing** — Next.js masks thrown server-action errors in prod, so throwing would hide the real reason from the cancel modal.
