...
9. **`paymentType` schema sync (CRITICAL):** `paymentTypeSchema` (zod enum) in [actions/index.ts](src/app/actions/index.ts) MUST contain every string the webhook writes — i.e. `"<Tier> Monthly"` / `"<Tier> Annually"` with the duration suffix. The webhook builds `${plan.type} ${Monthly|Annually}` where `plan.type` comes from `PRICE_ID_TO_PLAN`; if the enum lacks that exact value, `updatePaymentType` returns `{ error }` and **silently never writes** — paid users never get upgraded and `paddleSubscriptionId` is never stored. (This bug shipped 2026-06-02 → 2026-06-18.) The full valid set: `Free One Month`, `Free`, `Expired`, and `Standard/Premium {Monthly,Annually}` plus the **asymmetric admin tier** `Admin Monthly` + `Premium Admin Annually` (monthly maps to type `"Admin"`, annual to `"Premium Admin"` — intentional, keep them).
10. **Cancellation has two phases:** clicking Cancel calls Paddle's end-of-period cancel and sets `subscriptionCanceledAt` on the user immediately (access continues). The `subscription.canceled` webhook fires only at **period end**, flipping `paymentType → "Expired"` and clearing both `paddleSubscriptionId` and `subscriptionCanceledAt`. The profile card uses `subscriptionCanceledAt` to show "Access until …" + a disabled "already cancelled" button during the gap.
